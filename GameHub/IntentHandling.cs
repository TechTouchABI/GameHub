    ...
    public partial class IRCExample : Form
    {
        ...
        private static Dictionary<string, Func<IRCExample, string, string, string, string>> RequestHandlers =
            new Dictionary<string, Func<IRCExample, string, string, string, string>>()
            {
                ...
                { "GamehubIntent",  ((thisObj, target, topic, requestbody) => thisObj.Gamehub(target,topic,requestbody))}
            };
        ...
        private string Gamehub(string target, string topic, string requestbody)
        {
            ...
            if (response == null)
            {
                ...
                {
                    speechText = PerformArrange(app, appA, appB, GamehubActions.DoFourbyFour);
                }
                else if (string.Equals("up and down", action, StringComparison.OrdinalIgnoreCase))
                {
                    speechText = PerformArrange(app, appA, appB, GamehubActions.DoUpAndDown);
                }
                else if (string.Equals("left and rights", action, StringComparison.OrdinalIgnoreCase) ||
                    string.Equals("left and rights", action, StringComparison.OrdinalIgnoreCase))
                {
                    speechText = PerformArrange(app, appA, appB, GamehubActions.DoLeftAndRights);
                }
        ...
        private string PerformGamehubActions(string application, string appA, string appB, GamehubActions actionName)
        {
            int result = 0;
            string speechText = "";
            if (!string.IsNullOrEmpty(appA) && !string.IsNullOrEmpty(appB))
            {
                ...
                result = winCTL.ArrangeWindows(actionName, appNames);
                if (result > 0)
                    speechText = string.Format(" " + result + " " + appA + " and " + appB + " windows.");
                else
                    speechText = string.Format("I'm sorry.  Please make sure Gamehub application is on system ");
            }
            else
            {
                string applicationName = "";
                ...
                result = winCTL.ArrangeWindows(actionName, applicationName);
                if (result > 0)
                    speechText = string.Format("Executed " + result + " " + applicationName + " windows.");
                else
                    speechText = string.Format("I'm sorry.  Please make sure Gamehub application is on system ");
            }
            return speechText;
